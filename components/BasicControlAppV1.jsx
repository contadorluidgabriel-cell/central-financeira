...
export default function BasicControlAppV1({ embedded = false } = {}) {
  ...
  const [filters, setFilters] = useState({ category: '', customer: '', supplier: '', payment: '', status: '', min: '', max: '', sort: 'date_desc' });
  ...
  return <div className={styles.shell} style={embedded ? {minHeight:0,background:'transparent'} : undefined}>
    {!embedded && <aside className={styles.sidebar}>
      ...
      <nav className={styles.nav}>
        ...
        <NavButton active={false} icon="download" label="Importações" onClick={() => window.location.assign('/importacoes')}/>
    ...
    <main className={styles.main} style={embedded ? {marginLeft:0,minHeight:0,width:'100%'} : undefined}>
      ...
      <div className={styles.content} style={embedded ? {padding:0,maxWidth:'none'} : undefined}>
        ...
        {view === 'customers' && <PartyPage title="Clientes" kind="customer" parties={customers} entries={allCompanyEntries.filter(e => e.type === 'revenue')} onCreate={() => setModal({ type: 'customer' })} onOpen={party => setModal({ type: 'customerDetail', party })} onEdit={party => setModal({ type: 'customer', party })} onToggle={party => run(() => setSimpleCustomerActive(party.id, !party.active), party.active ? 'Cliente inativado.' : 'Cliente reativado.')} onDelete={party => run(() => deleteUnusedSimpleCustomer(party.id), 'Cliente excluído.')} onMerge={() => setModal({ type: 'customerMerge' })}/>} 
        {view === 'suppliers' && <PartyPage title="Fornecedores" kind="supplier" parties={suppliers} entries={allCompanyEntries.filter(e => e.type === 'expense')} onCreate={() => setModal({ type: 'supplier' })} onOpen={party => setModal({ type: 'supplierDetail', party })} onEdit={party => setModal({ type: 'supplier', party })} onToggle={party => run(() => setBasicSupplierActive(party.id, !party.active), party.active ? 'Fornecedor inativado.' : 'Fornecedor reativado.')} onDelete={party => run(() => deleteUnusedBasicSupplier(party.id), 'Fornecedor excluído.')} onMerge={() => setModal({ type: 'supplierMerge' })}/>} 
        ...
        {view === 'settings' && <Settings company={company} currentMonth={state.currentMonth} revenueCategories={categoriesRevenue} expenseCategories={categoriesExpense} paymentMethods={paymentMethods}
          onEntryMode={mode => run(() => scheduleUnifiedEntryMode(company.id, mode, nextMonth(state.currentMonth)), `Mudança de lançamentos programada para ${monthName(nextMonth(state.currentMonth))}.`)}
    ...
    {modal && <Modal title={modalTitle(modal.type)} wide={modal.type === 'revenueImport' || modal.type === 'expenseImport'} onClose={() => !busy && setModal(null)}>
      ...
      {modal.type === 'expense' && <ExpenseForm mode={expenseMode} entry={modal.entry} draft={modal.draft} categories={categoriesExpense.filter(c => c.active)} suppliers={suppliers.filter(s => s.active)} paymentMethods={paymentMethods.filter(p => p.active)} busy={busy} onQuickSupplier={draft => setModal({ type: 'supplier', returnToExpense: true, expenseDraft: draft })} onSubmit={async values => { const result = await run(() => saveBasicExpense({ organizationId: company.id, month, mode: expenseMode, existingId: modal.entry?.id, ...values }), modal.entry ? 'Despesa atualizada.' : 'Despesa registrada.'); if (result) setModal(null); }}/>} 
...
function PartyPage({ title, kind, parties, entries, onCreate, onOpen, onEdit, onToggle, onDelete, onMerge }) {
...
function Settings({ company, currentMonth, revenueCategories, expenseCategories, paymentMethods, onEntryMode, onTier, onRevenueCategory, onExpenseCategory, onToggleRevenueCategory, onToggleExpenseCategory, onPayment, onTogglePayment }) {
...
function BasicMobileNav({ view, revenueDetailed, expenseDetailed, onNavigate }) {
  ...
  return <><div className={styles.mobileNav}>{items.map(([id, icon, label]) => <button key={id} className={view === id ? styles.mobileActive : ''} onClick={() => { if (id === 'more') setMoreOpen(true); else onNavigate(id); }}><Icon name={icon}/><span>{label}</span></button>)}</div>{moreOpen && <div className={styles.mobileMoreBackdrop} onClick={() => setMoreOpen(false)}><div className={styles.mobileMore} onClick={e => e.stopPropagation()}>
    ...
    <button onClick={() => window.location.assign('/importacoes')}><Icon name="download"/>Importações</button>
  </div></div>}</>;
}

function modalTitle(type) { return ({revenue:'Receita',expense:'Despesa',revenueImport:'Importar receitas',expenseImport:'Importar despesas',customer:'Cliente',supplier:'Fornecedor',customerDetail:'Detalhes do cliente',supplierDetail:'Detalhes do fornecedor',customerMerge:'Mesclar clientes',supplierMerge:'Mesclar fornecedores',revenueCategory:'Categoria de receita',expenseCategory:'Categoria de despesa',payment:'Meio de pagamento',revenueAdjustment:'Ajuste de faturamento',expenseAdjustment:'Ajuste de despesa',revenueCancel:'Cancelar/estornar receita',expenseCancel:'Cancelar/estornar despesa',expenseBulk:'Ações em massa',reopen:'Reabrir mês'})[type] || 'Central Financeira'; }
