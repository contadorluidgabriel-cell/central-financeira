...
const MODULE_META = [
  ...
  ['accounts', 'Contas e caixa', 'Veja saldos por banco, caixa ou carteira.'],
  ['installments', 'Parcelamentos', 'Controle financiamentos, empréstimos e parcelas.'],
  ['recurring', 'Recorrências', 'Cadastre compromissos que se repetem todo mês.'],
  ['cashflow', 'Fluxo de caixa', 'Projete o caixa usando valores futuros.'],
  ['budgets', 'Orçamento', 'Compare o planejado com o realizado.']
];
...
export default function CentralFinanceiraV2() {
  ...
  const openWorkspace = (id, view = 'overview') => setState(s => ({ ...s, selectedCompany: id, workspaceView: view }));
  const closeWorkspace = () => setState(s => ({ ...s, selectedCompany: isMaster ? null : s.selectedCompany, workspaceView: 'overview' }));
  const setWorkspaceView = view => setState(s => ({ ...s, workspaceView: view }));
  ...
  function Overview({ c }) {
    ...
    if (s.projected < 0 && modules.cashflow) attention.push({ icon: 'chart', title: 'Projeção de caixa negativa', text: `Projeção até ${formatDate(s.horizon)}: ${money(s.projected)}`, view: 'cashflow' });
    if (missingFor(c).length && !submissionFor(c.id)) attention.push({ icon: 'file', title: 'Competência ainda incompleta', text: `Falta informar ${missingFor(c).join(' e ')}.`, view: 'competence' });
    return <>
      ...
      <section className="panel"><div className="panelhead"><div><h3>Ações rápidas</h3><p>Registre sem precisar navegar pelo sistema.</p></div></div><div className="panelbody"><div className={styles.quickActions}><button className={styles.quickAction} onClick={() => { setTransactionType('revenue'); setModal({ type: 'entry', companyId: c.id, entryType: 'revenue' }); }}><div className={styles.quickIcon}><Icon name="up"/></div><div><strong>Receita</strong><span>Registrar entrada</span></div></button>{modules.expenses && <button className={styles.quickAction} onClick={() => { setTransactionType('expense'); setModal({ type: 'entry', companyId: c.id, entryType: 'expense' }); }}><div className={styles.quickIcon}><Icon name="down"/></div><div><strong>Despesa</strong><span>Registrar gasto</span></div></button>}{modules.receivables && <button className={styles.quickAction} onClick={() => setModal({ type: 'receivable', companyId: c.id })}><div className={styles.quickIcon}><Icon name="calendar"/></div><div><strong>A receber</strong><span>Novo recebimento futuro</span></div></button>}{modules.payables && <button className={styles.quickAction} onClick={() => setModal({ type: 'payable', companyId: c.id })}><div className={styles.quickIcon}><Icon name="calendar"/></div><div><strong>A pagar</strong><span>Novo compromisso</span></div></button>}</div></div></section>
      {upcoming.length > 0 && <section className={`panel ${styles.sectionGap}`}><div className="panelhead"><div><h3>Próximos compromissos</h3><p>Entradas e saídas previstas nos próximos 30 dias.</p></div><button className="btn btn-ghost" onClick={() => setWorkspaceView('cashflow')}>Ver fluxo <Icon name="arrow"/></button></div><div className="panelbody"><Timeline items={upcoming}/></div></section>}
    </>;
  }
  ...
  function Transactions({ c }) {
    ...
    return <>
      <PageHeader title="Movimentações" description="Receitas e despesas registradas na competência.">{!locked && <button className="btn btn-primary" onClick={() => setModal({ type: 'entry', companyId: c.id, entryType: type })}><Icon name="plus"/>{type === 'revenue' ? 'Nova receita' : 'Nova despesa'}</button>}</PageHeader>
      {locked && <div className="lockbar"><div><Icon name="lock"/></div><div><strong>{isClosed(c.id) ? 'Competência fechada' : 'Dados já confirmados'}</strong><p>Os lançamentos ficam bloqueados até uma reabertura.</p></div></div>}
  ...
  function Accounts({ c }) {
    ...
    return <><PageHeader title="Contas e caixa" description="Saldos financeiros e movimentações por conta."><button className="btn btn-secondary" onClick={() => setModal({ type: 'transfer', companyId: c.id })} disabled={accounts.length < 2}><Icon name="repeat"/>Transferir</button><button className="btn btn-primary" onClick={() => setModal({ type: 'account', companyId: c.id })}><Icon name="plus"/>Nova conta</button></PageHeader><div className={styles.cashAccounts}><div className={styles.balanceTotal}><span>Saldo total informado</span><strong>{money(total)}</strong></div>{accounts.map(account => <div key={account.id} className={styles.accountCard}><div className={styles.accountTop}><div><div className={styles.accountType}>{accountTypeName(account.type)}</div><h4>{account.name}</h4></div><div className={styles.rowActions}><button className={styles.iconButton} onClick={() => setModal({ type: 'account', companyId: c.id, record: account })}><Icon name="edit"/></button><button className={`${styles.iconButton} ${styles.dangerButton}`} onClick={() => { if (confirm('Desativar esta conta?')) run(() => deactivateAccount(account.id), 'Conta desativada'); }}><Icon name="trash"/></button></div></div><div className={styles.accountBalance}>{money(account.balance)}</div><small>Saldo inicial: {money(account.openingBalance)}</small></div>)}</div><section className={`panel ${styles.sectionGap}`}><div className="panelhead"><div><h3>Movimentações de caixa</h3><p>Entradas e saídas que alteram o saldo das contas.</p></div>{accounts.length > 0 && <button className="btn btn-secondary" onClick={() => setModal({ type: 'movement', companyId: c.id })}><Icon name="plus"/>Ajuste manual</button>}</div>{movements.length ? <div className="tablewrap"><table className={styles.financialTable}><thead><tr><th>Data</th><th>Conta</th><th>Descrição</th><th>Origem</th><th className={styles.right}>Valor</th></tr></thead><tbody>{movements.map(item => { const acc = accounts.find(a => a.id === item.accountId); return <tr key={item.id}><td data-label="Data">{formatDate(item.date)}</td><td data-label="Conta">{acc?.name || 'Conta'}</td><td data-label="Descrição">{item.description}</td><td data-label="Origem">{({ receivable:'Recebimento',payable:'Pagamento',installment:'Parcela',manual:'Manual',transfer:'Transferência' })[item.sourceType] || 'Movimento'}</td><td data-label="Valor" className={`${styles.right} ${item.direction === 'in' ? styles.amountIn : styles.amountOut}`}>{item.direction === 'in' ? '+' : '−'} {money(item.amount)}</td></tr>; })}</tbody></table></div> : <EmptyState icon="wallet" title="Sem movimentações" text="Baixas de recebimentos, pagamentos e ajustes aparecerão aqui."/>}</section></>;
  }
  ...
  function CashFlow({ c }) {
    ...
    return <><PageHeader title="Fluxo de caixa" description="Saldo atual combinado com entradas e saídas previstas."/><div className={styles.metricGrid}><div className={styles.metric}><span>Saldo em contas</span><strong>{money(s.accountBalance)}</strong><small>Base atual</small></div><div className={styles.metric}><span>A receber</span><strong>{money(s.receivable)}</strong><small>Valores ainda não recebidos</small></div><div className={styles.metric}><span>A pagar</span><strong>{money(s.payable)}</strong><small>Contas ainda não pagas</small></div><div className={`${styles.metric} ${styles.metricEmphasis}`}><span>Projeção {profileFor(c.id).defaultProjectionDays} dias</span><strong className={s.projected < 0 ? 'negative' : 'positive'}>{money(s.projected)}</strong><small>Inclui parcelas previstas</small></div></div><div className="grid sectiongrid"><section className="panel span5"><div className="panelhead"><div><h3>Projeções</h3><p>Como o caixa pode evoluir.</p></div></div><div className="panelbody">{projections.map(row => <div className={styles.forecastLine} key={row.days}><span>{row.days} dias</span><div className={styles.forecastBar}><div style={{ width: `${Math.max(4, Math.min(100, Math.abs(row.value) / max * 100))}%` }}/></div><strong className={row.value < 0 ? 'negative' : 'positive'}>{money(row.value)}</strong></div>)}</div></section><section className="panel span7"><div className="panelhead"><div><h3>Agenda financeira</h3><p>Próximas entradas e saídas.</p></div></div><div className="panelbody">{upcoming.length ? <Timeline items={upcoming.slice(0, 10)}/> : <EmptyState icon="calendar" title="Sem compromissos futuros" text="Cadastre contas a pagar, receber ou parcelamentos para alimentar a projeção."/>}</div></section></div></>;
  }

  function Installments({ c }) {
    const plans = plansFor(c.id); const installments = installmentsFor(c.id);
    return <><PageHeader title="Parcelamentos" description="Financiamentos, empréstimos e compras parceladas do negócio."><button className="btn btn-primary" onClick={() => setModal({ type: 'plan', companyId: c.id })}><Icon name="plus"/>Novo parcelamento</button></PageHeader>{plans.length ? <div className={styles.planGrid}>{plans.map(plan => { const rows = installments.filter(x => x.planId === plan.id).sort((a,b)=>a.number-b.number); const paid = rows.reduce((s,x)=>s+x.paidAmount,0); const total = rows.reduce((s,x)=>s+x.amount,0) || plan.totalAmount; const pct = total ? Math.min(100, paid/total*100) : 0; const next = rows.find(x=>x.status!=='paid'); return <div className={styles.planCard} key={plan.id}><div className={styles.planHeader}><div><h4>{plan.description}</h4><p>{planTypeName(plan.type)} • {plan.creditor || 'Sem credor'}</p></div><span className={styles.statusPill}>{rows.filter(x=>x.status==='paid').length}/{rows.length}</span></div><div className={styles.planValue}>{money(Math.max(0,total-paid))}</div><div className={styles.progressTrack}><div style={{width:`${pct}%`}}/></div><div className={styles.planFooter}><span>Pago {money(paid)}</span><span>{pct.toFixed(0)}%</span></div><div className={styles.installmentList}>{rows.slice(0,6).map(inst => <div className={styles.installmentRow} key={inst.id}><span>#{inst.number}</span><div><strong>{formatDate(inst.dueDate)}</strong><small style={{display:'block'}}>{statusName(inst.status)}</small></div><strong>{money(Math.max(0,inst.amount-inst.paidAmount))}</strong>{inst.status!=='paid' ? <button className={styles.iconButton} onClick={()=>setModal({type:'settleInstallment',companyId:c.id,record:inst,plan})}><Icon name="check"/></button> : <Icon name="check"/>}</div>)}</div>{next && rows.length>6 && <button className="btn btn-ghost" style={{marginTop:8}}>Mais {rows.length-6} parcela(s)</button>}</div>; })}</div> : <EmptyState icon="list" title="Nenhum parcelamento cadastrado" text="Controle saldo devedor, parcelas e pagamentos de financiamentos ou empréstimos." action={()=>setModal({type:'plan',companyId:c.id})} actionLabel="Novo parcelamento"/>}</>;
  }

  function Recurring({ c }) {
    const items = recurringFor(c.id);
    return <><PageHeader title="Recorrências" description="Cadastre compromissos que se repetem e gere o mês sem redigitar."><button className="btn btn-secondary" disabled={!items.length || busy} onClick={async()=>{const ids=await run(()=>generateRecurringMonth({organizationId:c.id,month:state.month,items:state.recurringItems}),null);if(ids)setToast(ids.length?`${ids.length} compromisso(s) gerado(s)`:'Nada novo para gerar neste mês')}}><Icon name="refresh"/>Gerar {shortMonth(state.month)}</button><button className="btn btn-primary" onClick={()=>setModal({type:'recurring',companyId:c.id})}><Icon name="plus"/>Nova recorrência</button></PageHeader><section className="panel">{items.length ? <div className="tablewrap"><table className={styles.financialTable}><thead><tr><th>Tipo</th><th>Descrição</th><th>Parte</th><th>Dia</th><th className={styles.right}>Valor</th><th/></tr></thead><tbody>{items.map(item=><tr key={item.id}><td data-label="Tipo"><span className={`${styles.statusPill} ${item.direction==='receivable'?styles.statusPaid:styles.statusPending}`}>{item.direction==='receivable'?'A receber':'A pagar'}</span></td><td data-label="Descrição"><strong>{item.description}</strong></td><td data-label="Parte">{item.counterparty||'—'}</td><td data-label="Dia">Dia {item.dayOfMonth}</td><td data-label="Valor" className={styles.right}>{money(item.amount)}</td><td data-label="Ações"><div className={styles.rowActions}><button className={styles.iconButton} onClick={()=>setModal({type:'recurring',companyId:c.id,record:item})}><Icon name="edit"/></button><button className={`${styles.iconButton} ${styles.dangerButton}`} onClick={()=>{if(confirm('Desativar esta recorrência?'))run(()=>deactivateRecurringItem(item.id),'Recorrência desativada')}}><Icon name="trash"/></button></div></td></tr>)}</tbody></table></div> : <EmptyState icon="repeat" title="Nenhuma recorrência" text="Aluguel, mensalidades, contratos e receitas recorrentes podem ser cadastrados uma vez." action={()=>setModal({type:'recurring',companyId:c.id})} actionLabel="Nova recorrência"/>}</section></>;
  }
  ...
  function Competence({ c }) {
    ...
    return <><PageHeader title="Competência mensal" description="Conferência, envio, análise e fechamento do mês."/>
  ...
  function Workspace({ c }) {
    ...
    })[state.workspaceView] || <Overview c={c}/>;
    return <><div className={styles.workspaceHeader}><div className={styles.workspaceIdentity}>{isMaster && <button className={styles.workspaceBack} onClick={closeWorkspace}><Icon name="back"/>Carteira</button>}<div className="avatar">{initials(c.name)}</div><div><h2>{c.name}</h2><p>{c.document || 'Documento não informado'} • {monthName(state.month)}</p></div></div><div className="statusStrip"><span className={`status ${competenceStatus(c.id)[1]}`}>{competenceStatus(c.id)[0]}</span></div></div><div className={styles.moduleNav}>{nav.map(([key,icon,label])=><button key={key} className={`${styles.moduleButton} ${state.workspaceView===key?styles.moduleButtonActive:''}`} onClick={()=>setWorkspaceView(key)}><Icon name={icon}/>{label}</button>)}</div>{content}<div className={styles.mobileNav}>{nav.filter(x=>['overview','transactions','receivables','payables','competence'].includes(x[0])).slice(0,5).map(([key,icon,label])=><button key={key} className={state.workspaceView===key?styles.mobileActive:''} onClick={()=>setWorkspaceView(key)}><Icon name={icon}/>{label}</button>)}</div></>;
  }
