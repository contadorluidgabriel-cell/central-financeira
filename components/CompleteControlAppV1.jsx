...
export default function CompleteControlAppV1() {
  ...
  const entries = useMemo(() => state?.entries.filter(item => item.companyId === selectedCompanyId) || [], [state, selectedCompanyId]);
  ...
  const linkableEntries = useMemo(() => entries.map(entry => {
    const allocated = obligations
      .filter(item => item.financialEntryId === entry.id)
      .reduce((sum, item) => sum + (item.status === 'cancelled' ? item.settledAmount : item.originalAmount), 0);
    return { ...entry, allocated, allocationRemaining: Math.max(0, entry.amount - allocated) };
  }).filter(entry => entry.allocationRemaining > 0.00001), [entries, obligations]);
...
function ObligationModal({ direction, entries, customers, suppliers, categories, busy, onClose, onSave }) {
  const [form, setForm] = useState({ origin: 'manual', financialEntryId: '', description: '', issueDate: localToday(), dueDate: localToday(), amount: '', customerId: '', supplierId: '', categoryId: '', notes: '' });
  const patch = (key, value) => setForm(prev => ({ ...prev, [key]: value }));
  const counterparties = direction === 'receivable' ? customers : suppliers;
  const selectedEntry = entries.find(item => item.id === form.financialEntryId) || null;
  ...
  function chooseEntry(id) {
    ...
    setForm(prev => ({
      ...
      issueDate: entry?.date ? String(entry.date).slice(0, 10) : prev.issueDate,
      dueDate: entry?.date && prev.dueDate < String(entry.date).slice(0, 10) ? String(entry.date).slice(0, 10) : prev.dueDate,
      amount: entry ? String(entry.allocationRemaining) : prev.amount,
      customerId: direction === 'receivable' && entry?.customerId ? entry.customerId : prev.customerId,
      supplierId: direction === 'payable' && entry?.supplierId ? entry.supplierId : prev.supplierId,
      categoryId: entry?.categoryId || prev.categoryId
    }));
  }

  return <Modal title={direction === 'receivable' ? 'Nova conta a receber' : 'Nova conta a pagar'} description="Pode nascer de um lançamento da competência, ser um novo compromisso ou um saldo anterior." onClose={onClose}><form className={styles.form} onSubmit={event => { event.preventDefault(); onSave({ ...form, financialEntryId: form.origin === 'financial_entry' ? form.financialEntryId || null : null, customerId: direction === 'receivable' ? form.customerId || null : null, supplierId: direction === 'payable' ? form.supplierId || null : null, categoryId: form.categoryId || null }); }}><div className={styles.formGrid}><Field label="Origem"><select value={form.origin} onChange={e => setForm(prev => ({ ...prev, origin: e.target.value, financialEntryId: e.target.value === 'financial_entry' ? prev.financialEntryId : '' }))}><option value="manual">Novo compromisso</option><option value="financial_entry">Vincular à competência</option><option value="opening">Saldo de abertura</option></select></Field><Field label={direction === 'receivable' ? 'Cliente' : 'Fornecedor'}><select value={direction === 'receivable' ? form.customerId : form.supplierId} onChange={e => patch(direction === 'receivable' ? 'customerId' : 'supplierId', e.target.value)}><option value="">Não informado</option>{counterparties.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field></div>{form.origin === 'financial_entry' && <><Field label="Lançamento da competência"><select required value={form.financialEntryId} onChange={e => chooseEntry(e.target.value)}><option value="">Selecione um lançamento</option>{entries.map(item => <option key={item.id} value={item.id}>{item.month} • {item.description} • disponível {money(item.allocationRemaining)}</option>)}</select></Field>{selectedEntry && <div className={styles.linkedEntryNote}><strong>{money(selectedEntry.allocationRemaining)} disponível para vincular</strong><span>Valor original {money(selectedEntry.amount)} • já vinculado {money(selectedEntry.allocated)}</span></div>}</>}<Field label="Descrição"><input required value={form.description} onChange={e => patch('description', e.target.value)} placeholder={direction === 'receivable' ? 'Ex.: Serviço mensal' : 'Ex.: Aluguel'}/></Field><div className={styles.formGrid}><Field label="Emissão"><input type="date" value={form.issueDate} onChange={e => patch('issueDate', e.target.value)}/></Field><Field label="Vencimento"><input required type="date" value={form.dueDate} min={form.issueDate || undefined} onChange={e => patch('dueDate', e.target.value)}/></Field></div><div className={styles.formGrid}><Field label="Valor"><input required min="0.01" max={selectedEntry?.allocationRemaining || undefined} type="number" step="0.01" value={form.amount} onChange={e => patch('amount', e.target.value)}/></Field><Field label="Categoria"><select value={form.categoryId} onChange={e => patch('categoryId', e.target.value)}><option value="">Sem categoria</option>{categories.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field></div><Field label="Observação"><textarea value={form.notes} onChange={e => patch('notes', e.target.value)} rows="3"/></Field><ModalActions busy={busy} onClose={onClose}/></form></Modal>;
}
