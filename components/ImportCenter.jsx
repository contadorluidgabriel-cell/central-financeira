...
export default function ImportCenter() {
  ...
  return <main className={styles.page}>
    <div className={styles.container}>
      ...
      <nav className={styles.tabs} aria-label="Tipos de importação">{[['spreadsheets','Planilhas Excel/CSV'],['ofx','Extrato OFX'],['posting','Lançar do OFX'],['history','Histórico OFX']].map(([value, label]) => <button type="button" key={value} aria-current={tab === value ? 'page' : undefined} className={tab === value ? styles.activeTab : ''} onClick={() => { setPostingAutoOpen(false); setTab(value); setError(''); setNotice(''); }}>{label}</button>)}</nav>
      ...
      {tab === 'ofx' && <section className={styles.panel}><div className={styles.sectionHeader}><div><div className={styles.eyebrow}>ETAPA 1 · ARQUIVO BANCÁRIO</div><h2>Importar extrato OFX</h2><p>O arquivo entra como movimentações para conferência. Não altera faturamento nem saldo ao ser enviado.</p></div></div><div className={styles.ofxTool}><OfxImportLauncher preferredOrganizationId={organizationId} onReviewImported={importedOrganizationId => { setOrganizationId(importedOrganizationId); setPostingAutoOpen(true); setTab('posting'); setError(''); setNotice(''); }}/></div></section>}
      {tab === 'posting' && <section className={styles.panel}><div className={styles.sectionHeader}><div><div className={styles.eyebrow}>ETAPA 2 · CONFERÊNCIA</div><h2>Criar lançamentos a partir do OFX</h2><p>Prepare até 100 movimentos em lote ou escolha individualmente, confira os valores e confirme. Movimentos manuais anteriores exigem atenção para evitar duplicidades.</p></div></div><div className={styles.ofxTool}><OfxEntryPostingLauncher autoOpen={postingAutoOpen} preferredOrganizationId={organizationId}/></div></section>}
