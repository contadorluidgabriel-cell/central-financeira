...
const ALIASES = {
  ...
  date: ['data', 'data despesa', 'data da despesa', 'data do gasto', 'data gasto', 'data emissao', 'data de emissao', 'data movimento', 'data do movimento'],
  amount: ['valor', 'valor total', 'valor do dia', 'valor despesa', 'valor da despesa', 'despesa', 'total', 'total despesa', 'vlr'],
  description: ['descricao', 'descricao da despesa', 'descricao despesa', 'observacao', 'historico', 'detalhe', 'detalhes'],
  category: ['categoria', 'categoria despesa', 'categoria da despesa', 'tipo despesa', 'grupo'],
  supplier: ['fornecedor', 'nome do fornecedor', 'razao social', 'nome razao social', 'favorecido', 'beneficiario'],
  document: ['cpf cnpj', 'cpf cnpj fornecedor', 'cpf', 'cnpj', 'documento', 'documento fornecedor'],
...
function findColumn(header, key) {
  ...
  const heuristics = {
    ...
    date: item => item === 'data' || (item.includes('data') && /(despesa|gasto|emissao|movimento)/.test(item)),
    amount: item => /(^| )(valor|total|despesa)( |$)/.test(item) && !item.includes('desconto'),
    description: item => item.includes('descricao') || item.includes('historico') || item.includes('observacao') || item.includes('detalhe'),
    category: item => item.includes('categoria') || item === 'grupo',
    supplier: item => item.includes('fornecedor') || item.includes('razao social') || item.includes('favorecido') || item.includes('beneficiario'),
...
export async function parseBasicExpenseImport({ file, mode, month, existingEntries = [], categories = [], suppliers = [], paymentMethods = [] }) {
  ...
  if (mode !== 'monthly' && columns.date < 0) throw new Error('Não encontrei a coluna de data. Use “Data”, “Data da despesa” ou o modelo da Central.');

  const columnWarnings = [];
  if (mode === 'individual') {
    if (columns.supplier < 0) columnWarnings.push('Não identifiquei uma coluna de fornecedor. As linhas sem fornecedor serão vinculadas a Fornecedor não informado.');
    if (columns.category < 0) columnWarnings.push('Não identifiquei uma coluna de categoria. As despesas serão importadas sem categoria.');
    if (columns.payment < 0) columnWarnings.push('Não identifiquei uma coluna de meio de pagamento. As despesas serão importadas como “Não informado”.');
    if (columns.document < 0 && columns.supplier >= 0) columnWarnings.push('Não identifiquei CPF/CNPJ. A identificação de fornecedores será feita apenas pelo nome.');
  }
  ...
  return { fileName: file.name, sheetName, rows, columnWarnings, headers: header };
}
