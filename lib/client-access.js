...
export async function resetClientPassword({ userId, email, document = '' }) {
  ...
  ensureOk(passwordResult, 'Não foi possível redefinir a senha.');

  const markResult = await neonTest.rpc('mark_client_password_temporary', { p_user_id: userId });
  if (markResult?.error) throw new Error(messageFrom(markResult.error, 'A senha foi alterada, mas não foi possível marcar a troca obrigatória.'));
