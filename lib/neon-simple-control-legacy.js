...
export async function loadSimpleControlData(activeOrganizationId = null) {
  ...
  const [settings, categories, customers, paymentMethods, entries, submissions, notifications] = await Promise.all([
    query('organization_settings', 'organization_id,revenue_mode,control_tier,control_start_month,pending_control_tier,pending_control_tier_effective,pending_revenue_mode,pending_revenue_mode_effective,pending_expense_mode,pending_expense_mode_effective,active'),
...
export async function updateSimpleSettings(organizationId, patch) {
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'revenueMode')) { payload.revenue_mode = patch.revenueMode; payload.expense_mode = patch.revenueMode; }
  ...
  if (Object.prototype.hasOwnProperty.call(patch, 'pendingRevenueMode')) { payload.pending_revenue_mode = patch.pendingRevenueMode || null; payload.pending_expense_mode = patch.pendingRevenueMode || null; }
