...
export function downloadSimpleImportTemplate(mode, month) {
  ...
  const example = mode === 'monthly'
    ? [month, '12500,00', 'Exemplo — apague esta linha antes de importar']
    : mode === 'daily'
      ? [`01/${month.slice(5, 7)}/${month.slice(0, 4)}`, '850,00', 'Exemplo — apague esta linha antes de importar']
      : [`01/${month.slice(5, 7)}/${month.slice(0, 4)}`, '350,00', 'Exemplo — apague esta linha antes de importar', 'Prestação de serviços', 'Cliente exemplo', '', 'Pix'];
